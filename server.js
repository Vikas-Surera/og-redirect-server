const express = require('express');
const app = express();
const port = process.env.PORT || 5000;
const path = require('path');
const fs = require('fs')

app.use(express.static(path.join(__dirname, 'public')));

app.get('/', function(request, response) {
  console.log('Home page visited!');
    const data = '<!DOCTYPE html><html lang="en"><head>'
        + '<meta property="og:title" content="This is title" />'
        + '<meta property="og:description" content="This is description" />'
        + '<meta property="og:image" content="https://codeat21.com/wp-content/uploads/2021/03/CodeAT21-Logo.png" />'
        + '<meta property="og:url" content="https://codeat21.com/wp-content/uploads/2021/03/CodeAT21-Logo.png" />'
        + '<meta property="og:type" content="article" />'
        + '<meta http-equiv="refresh" content="1; url=https://codeat21.com/wp-content/uploads/2021/03/CodeAT21-Logo.png" />'
        +'</head></html>';
    return response.send(data);
});


app.get('*', function(req, res) {
	res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

app.listen(port, () => console.log(`Listening on port ${port}`));
